...
se.ComponentPlatformPlayerController.prototype.update=function(a,b){this.deltaTime=b;var c=this.joystick.getAxis("horizontal")*this.speed*b,c=c||0,e=this.joystick.getAxis("jump");if(c||e){var d=0;c&&(0<c?a.transform.rotate.x=1:0>c&&(a.transform.rotate.x=-1));e&&(d=-10);a.rigidbody.setVelocity(c,d)}};se.ComponentPlatformPlayerController.prototype.isgrounded=function(){return!this.parent.transform.canMove(0,this.gravity*this.deltaTime)};
...
se.ComponentRigidBody.prototype.update=function(a,b){a.transform.move(0,this.gravity*b)};se.ComponentRigidBody.prototype.setParent=function(a){this.parent=a};se.Extent=function(a,b,c,e){this.min={x:a,y:b};this.max={x:c,y:e}};se.Extent.createEmpty=function(){return new se.Extent(Infinity,Infinity,-Infinity,-Infinity)};se.Extent.prototype.isEmpty=function(a){return this.max.x<this.min.x||this.max.y<this.min.y};se.Extent.prototype.intersects=function(a){return this.min.x<=a.max.x&&this.max.x>=a.min.x&&this.min.y<=a.max.y&&this.max.y>=a.min.y};
se.Extent.prototype.getIntersection=function(a){var b=se.Extent.createEmpty();if(this.intersects(a))b.min.x=this.min.x>a.min.x?this.min.x:a.min.x,b.min.y=this.min.y>a.min.y?this.min.y:a.min.y,b.max.x=this.max.x<a.max.x?this.max.x:a.max.x,b.max.y=this.max.y<a.max.y?this.max.y:a.max.y;else return null;return b};se.Extent.prototype.getWidth=function(){return this.max.x-this.min.x};se.Extent.prototype.getHeight=function(){return this.max.y-this.min.y};se.GameObject=function(a,b,c,e,d,f){this.name=a;this.transform=new se.Transform(this,b,c);this.width=e;this.height=d;this.components=[];this.colliders=[];this.parent=this.renderer=null;this.children=[];f=f||{};this.isStatic=f.isStatic||!1;this.isSleeping=f.isSleeping||!1;this.angle=0;this.rigidbody=f.rigidbody||null};se.GameObject.prototype.getX=function(){return this.transform.position.x};se.GameObject.prototype.getY=function(){return this.transform.position.y};se.GameObject.prototype.getWidth=function(){return this.width};
se.GameObject.prototype.getHeight=function(){return this.height};se.GameObject.prototype.getColliders=function(){return this.colliders};se.GameObject.prototype.setRenderer=function(a){a.setParent(this);this.renderer=a};se.GameObject.prototype.update=function(a,b){this.rigidbody&&this.rigidbody.update(a,b);for(var c in this.components)this.components[c].update(this,a);for(c in this.children)this.children[c].update(a)};
se.GameObject.prototype.render=function(a){var b=this.transform.position,c=this.transform.rotate;a.translate(b.x,b.y);a.rotate(this.angle);a.scale(c.x,1);this.renderer&&this.renderer.render(a);for(var e in this.children)this.children[e].render(a);a.scale(c.x,1);a.rotate(-this.angle);a.translate(-b.x,-b.y)};se.GameObject.prototype.addComponent=function(a){this.components.push(a);a.setParent(this);return this};
...
se.Transform.prototype.resolveCollision=function(a,b){var c=this.parent,e=c.parent.getObjs(),d=c.getColliders(),f;for(f in d){var g=d[f],k;for(k in e){var h=e[k];c!=h&&(h=g.getIntersection(h.getColliders()))&&(0!=a&&(this.position.x=0>a?this.position.x+(h.getWidth()+.01):this.position.x-(h.getWidth()+.01)),0!=b&&(this.position.y=0>b?this.position.y+(h.getHeight()+.01):this.position.y-(h.getHeight()+.01)))}}};
se.Transform.prototype.canMove=function(a,b){var c=this.parent,e=c.parent.getObjs(),d=c.getColliders(),f;for(f in d){var g=d[f].clone();g.setParent(c);g.x+=a;g.y+=b;for(var k in e){var h=e[k];if(c!=h&&g.isIntersect(h.getColliders()))return!1}}return!0};se.Transform.prototype.getXY=function(){var a=this.position.x,b=this.position.y,c=this.parent.parent;c instanceof se.GameObject&&(a+=c.transform.position.x,b+=c.transform.position.y);return{x:a,y:b}};
...
se.Scene.prototype.getHeight=function(){return this.parent.getHeight()};se.Scene.prototype.getCamera=function(){return this.camera};se.Scene.prototype.getObjs=function(){return this.objs};se.Scene.prototype.setParent=function(a){this.parent=a};se.Scene.prototype.add=function(a){this.objs.push(a);a.setParent(this)};se.Scene.prototype.update=function(a,b){for(var c in this.objs)this.objs[c].update(a,b)};
se.Scene.prototype._applyGravity=function(a,b){var c="undefined"!==typeof b.scale?b.scale:.001;if((0!==b.x||0!==b.y)&&0!==c)for(var e in a){var d=a[e];d.isStatic||d.isSleeping||(d=d.rigidbody,d.force.y+=d.mass*b.y*c,d.force.x+=d.mass*b.x*c)}};
se.Scene.prototype._resolveColliders=function(a){for(var b=[],c=0;c<a.length;c++){var e=a[c];if(!e.isStatic&&!e.isSleeping)for(var d=0;d<a.length;d++)if(c!=d){var f=a[d],g=this._checkCollision(e,f);g&&b.push(g)}}for(c in b)if(g=b[c],g.objB.isStatic||f.isSleeping)a=g.objA.rigidbody.velocity,e=g.objA.transform.position,e.y=0<a.y?e.y-(g.intersection.getHeight()+.01):e.y+(g.intersection.getHeight()+.01),a.x=0,a.y=0};
se.Scene.prototype._checkCollision=function(a,b){var c=a.getColliders(),e=b.getColliders(),d;for(d in c){var f=c[d];if(f.isIntersect(e))return new se.Collision(a,b,f.getIntersection(e))}};se.Scene.prototype._clearForces=function(a){for(var b in a){var c=a[b].rigidbody;c.force.x=0;c.force.y=0;c.torque=0}};se.Scene.prototype.render=function(a){this.updatePivot(a);this.clearframe(a);this.renderBackground(a);for(var b in this.objs)this.objs[b].render(a)};
...
se.StarEngine.prototype.update=function(a){var b=this.runner,c=b.timing,e=1,d;b.isFixed?d=b.delta:(d=a-b.timePrev||b.delta,b.timePrev=a,b.deltaHistory.push(d),b.deltaHistory=b.deltaHistory.slice(-b.deltaSampleSize),d=Math.min.apply(null,b.deltaHistory),d=d<b.deltaMin?b.deltaMin:d,d=d>b.deltaMax?b.deltaMax:d,e=d/b.delta,b.delta=d);0!==b.timeScalePrev&&(e*=c.timeScale/b.timeScalePrev);0===c.timeScale&&(e=0);b.timeScalePrev=c.timeScale;b.correction=e;b.frameCounter+=1;1E3<=a-b.counterTimestamp&&(b.fps=
(a-b.counterTimestamp)/1E3*b.frameCounter,b.counterTimestamp=a,b.frameCounter=0);a=this.getSceneCurrent();a.update(d,e);a.render(this.ctx)};se.StarEngine.prototype.run=function(){var a=this.runner,b=this;(function e(d){a.frameRequestId=window.requestAnimationFrame(e);d&&a.enabled&&b.update(d)})();return a};se.Vector=function(a,b){this.x=a;this.y=b};se.Vector.prototype.equals=function(a){return this.x==a.x&&this.y==a.y};se.Vector.prototype.clone=function(){return new se.Vector(this.x,this.y)};se.Vector.prototype.change=function(a,b){this.x=a;this.y=b};se.Vector.prototype.getMagnitude=function(){return Math.sqrt(this.x*this.x+this.y*this.y)};se.Vector.prototype.sub=function(a,b){var c;c=b?this:new se.Vector(0,0);c.x=this.x-a.x;c.y=this.y-a.y;return c};
