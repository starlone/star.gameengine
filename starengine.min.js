...
se.ComponentPlatformPlayerController.prototype.update=function(a,b){var c=this.joystick.getAxis("horizontal")*b/1E3,c=c||0,d=this.joystick.getAxis("jump");if(c||d){var e=a.rigidbody.body.velocity,f=0;c&&(0<c?a.transform.rotate.x=1:0>c&&(a.transform.rotate.x=-1));if(10<e.x||-10>e.x)c=0;d&&1>e.y&&-1<e.y&&(f=-.1);a.rigidbody.applyForce({x:0,y:0},{x:c,y:f})}};se.ComponentPlatformPlayerController.prototype.setParent=function(a){this.parent=a};se.ComponentFollowObject=function(a){this.obj_target=a};
...
se.Transform.prototype.change=function(a,b){this.position.change(a,b)};se.Transform.prototype.move=function(a,b){this.position.x+=a;this.position.y+=b;this.resolveCollision(a,b)};
...
se.RigidBody.prototype.setVelocity=function(a,b){var c=new se.Vector(a,b),d=this.parent.transform.position;this.positionPrev.x=d.x-c.x;this.positionPrev.y=d.y-c.y;this.velocity.x=c.x;this.velocity.y=c.y;this.speed=this.velocity.getMagnitude()};se.RigidBody.prototype.applyForce=function(a,b,c){a=this.parent.transform.position;this.force.x+=c.x;this.force.y+=c.y;this.torque+=(b.x-a.position.x)*c.y-(b.y-a.position.y)*c.x};se.RigidBodyMatter=function(a){this.options=a};se.RigidBodyMatter.prototype.createBody=function(){var a=this.parent,b=a.transform.position;options=this.options||{};var c=options.x||b.x,b=options.y||b.y,d=options.width||a.getWidth(),e=options.height||a.getHeight();this.body=Matter.Bodies.rectangle(c,b,d,e,{isStatic:a.isStatic,canRotate:a.canRotate})};
se.RigidBodyMatter.prototype.update=function(a){a=this.parent.transform.position;a.x=this.body.position.x;a.y=this.body.position.y;this.parent.angle=this.body.angle};se.RigidBodyMatter.prototype.setParent=function(a){this.parent=a;this.createBody()};se.RigidBodyMatter.prototype.setVelocity=function(a,b){Matter.Body.setVelocity(this.body,{x:a,y:b})};se.RigidBodyMatter.prototype.applyForce=function(a,b){Matter.Body.applyForce(this.body,a,b)};
Matter.Body.update=function(a,b,c,d){var e=Matter.Bounds,f=Matter.Vector,h=Matter.Vertices,k=Matter.Axes;b=Math.pow(b*c*a.timeScale,2);c=1-a.frictionAir*c*a.timeScale;var g=a.position.y-a.positionPrev.y;a.velocity.x=(a.position.x-a.positionPrev.x)*c*d+a.force.x/a.mass*b;a.velocity.y=g*c*d+a.force.y/a.mass*b;a.positionPrev.x=a.position.x;a.positionPrev.y=a.position.y;a.position.x+=a.velocity.x;a.position.y+=a.velocity.y;a.angularVelocity=a.canRotate?(a.angle-a.anglePrev)*c*d+a.torque/a.inertia*b:0;
a.anglePrev=a.angle;a.angle+=a.angularVelocity;a.speed=f.magnitude(a.velocity);a.angularSpeed=Math.abs(a.angularVelocity);for(d=0;d<a.parts.length;d++)b=a.parts[d],h.translate(b.vertices,a.velocity),0<d&&(b.position.x+=a.velocity.x,b.position.y+=a.velocity.y),0!==a.angularVelocity&&(h.rotate(b.vertices,a.angularVelocity,a.position),k.rotate(b.axes,a.angularVelocity),0<d&&f.rotateAbout(b.position,a.angularVelocity,a.position,b.position)),e.update(b.bounds,b.vertices,a.velocity)};se.Scene=function(a,b){this.parent=a;this.camera=new se.GameObject("MainCamera",0,0,0,0);this.pivot=new se.Transform(this,0,0);this.objs=[];this.add(this.camera);b?this.renderer=b:(this.renderer=new se.GradientRenderer("#8ED6FF","#004CB3"),this.renderer.setParent(this));this.matterengine=Matter.Engine.create(this.parent.element)};se.Scene.prototype.getWidth=function(){return this.parent.getWidth()};se.Scene.prototype.getHeight=function(){return this.parent.getHeight()};
se.Scene.prototype.getCamera=function(){return this.camera};se.Scene.prototype.getObjs=function(){return this.objs};se.Scene.prototype.setParent=function(a){this.parent=a};se.Scene.prototype.add=function(a){this.objs.push(a);a.setParent(this);a.rigidbody&&Matter.World.add(this.matterengine.world,a.rigidbody.body)};se.Scene.prototype.update=function(a,b){Matter.Engine.update(this.matterengine,a,b);for(var c in this.objs)this.objs[c].update(a,b)};
