...
se.GameObject.prototype.addCollider=function(a){this.colliders.push(a);a.setParent(this);return this};se.GameObject.prototype.setParent=function(a){this.parent=a};se.GameObject.prototype.addChild=function(a){this.children.push(a);a.setParent(this)};se.RigidBody=function(a,b){this.parent=a;b=b||{};this.force=new se.Vector(0,0);this.velocity=new se.Vector(0,0);this.speed=0;this.positionPrev=this.parent.transform.position.clone();this.frictionAir=.01;this.mass=b.mass||6.4;this.timeScale=1};
se.RigidBody.prototype.update=function(a){var b=this.parent.transform.position;a*=100;var c=Math.pow(1*a*this.timeScale,2);a=1-this.frictionAir*a*this.timeScale;var d=b.y-this.positionPrev.y;this.velocity.x=(b.x-this.positionPrev.x)*a*1+this.force.x/this.mass*c;this.velocity.y=d*a*1+this.force.y/this.mass*c;this.positionPrev.x=b.x;this.positionPrev.y=b.y;b.x+=this.velocity.x;b.y+=this.velocity.y;this.speed=this.velocity.getMagnitude()};
se.RigidBody.prototype.setVelocity=function(a,b){var c=new se.Vector(a,b),d=this.parent.transform.position;this.positionPrev.x=d.x-c.x;this.positionPrev.y=d.y-c.y;this.velocity.x=c.x;this.velocity.y=c.y;this.speed=this.velocity.getMagnitude()};se.RigidBody.prototype.applyForce=function(a,b,c){a=this.parent.transform.position;this.force.x+=c.x;this.force.y+=c.y;this.torque+=(b.x-a.position.x)*c.y-(b.y-a.position.y)*c.x};se.Vector=function(a,b){this.x=a;this.y=b};
se.Vector.prototype.equals=function(a){return this.x==a.x&&this.y==a.y};se.Vector.prototype.clone=function(){return new se.Vector(this.x,this.y)};se.Vector.prototype.change=function(a,b){this.x=a;this.y=b};se.Vector.prototype.getMagnitude=function(){return Math.sqrt(this.x*this.x+this.y*this.y)};se.Transform=function(a,b,c){this.parent=a;this.position=new se.Vector(b,c);this.rotate=new se.Vector(1,0)};se.Transform.prototype.change=function(a,b){this.position.change(a,b)};
se.Transform.prototype.move=function(a,b){this.position.x+=a;this.position.y+=b;this.resolveCollision(a,b)};
se.Transform.prototype.resolveCollision=function(a,b){var c=this.parent,d=c.parent.getObjs(),e=c.getColliders(),f;for(f in e){var h=e[f],k;for(k in d){var g=d[k];c!=g&&(g=h.getIntersection(g.getColliders()))&&(0!=a&&(this.position.x=0>a?this.position.x+(g.getWidth()+.01):this.position.x-(g.getWidth()+.01)),0!=b&&(this.position.y=0>b?this.position.y+(g.getHeight()+.01):this.position.y-(g.getHeight()+.01)))}}};
se.Transform.prototype.canMove=function(a,b){var c=this.parent,d=c.parent.getObjs(),e=c.getColliders(),f;for(f in e){var h=e[f].clone();h.setParent(c);h.x+=a;h.y+=b;for(var k in d){var g=d[k];if(c!=g&&h.isIntersect(g.getColliders()))return!1}}return!0};se.Transform.prototype.getXY=function(){var a=this.position.x,b=this.position.y,c=this.parent.parent;c instanceof se.GameObject&&(a+=c.transform.position.x,b+=c.transform.position.y);return{x:a,y:b}};se.Joystick=function(){this.jump=!1;this.keyhandler=new se.KeyboardHandler(this)};se.Joystick.prototype.getAxis=function(a){return"horizontal"==a?this.x:"vertical"==a?this.y:"jump"==a?this.jump:0};se.Joystick.prototype.setAxis=function(a,b){"horizontal"==a&&(this.x=b);"vertical"==a&&(this.y=b);"jump"==a&&(this.jump=b)};se.Joystick.prototype.resetAxis=function(){this.y=this.x=0;this.jump=!1};
...
se.KeyboardHandler.prototype.keyup=function(a){"65"==a||"68"==a||"37"==a||"39"==a?this.joy.setAxis("horizontal",0):"87"==a||"83"==a?this.joy.setAxis("vertical",0):"32"==a&&this.joy.setAxis("jump",!1)};se.BoxRenderer=function(a){this.color=a};se.BoxRenderer.prototype.render=function(a){var b=this.parent;a.fillStyle=this.color;a.fillRect(0,0,b.width,b.height)};se.BoxRenderer.prototype.setParent=function(a){this.parent=a};se.ImageRenderer=function(a){this.img=new Image;this.is_load=!1;this.img.src=a};se.ImageRenderer.prototype.render=function(a){var b=this.parent;a.drawImage(this.img,0,0,b.width,b.height)};se.ImageRenderer.prototype.setParent=function(a){this.parent=a};se.Scene=function(a){this.camera=new se.GameObject("MainCamera",0,0,0,0);this.camera.setParent(this);this.pivot=new se.Transform(this,0,0);this.objs=[this.camera];this.backcolor=a||"rgba(135, 206, 250,1)";this.gravity={x:0,y:1,scale:.001}};se.Scene.prototype.getWidth=function(){return this.parent.getWidth()};se.Scene.prototype.getHeight=function(){return this.parent.getHeight()};se.Scene.prototype.getCamera=function(){return this.camera};se.Scene.prototype.getObjs=function(){return this.objs};
se.Scene.prototype.setParent=function(a){this.parent=a};se.Scene.prototype.add=function(a){this.objs.push(a);a.setParent(this)};se.Scene.prototype.update=function(a){this._applyGravity(this.objs,this.gravity);for(var b in this.objs){var c=this.objs[b];c.update(a);c.rigidbody.update(a)}};
se.Scene.prototype._applyGravity=function(a,b){var c="undefined"!==typeof b.scale?b.scale:.001;if((0!==b.x||0!==b.y)&&0!==c)for(var d in a){var e=a[d];e.isStatic||e.isSleeping||(e=e.rigidbody,e.force.y+=e.mass*b.y*c,e.force.x+=e.mass*b.x*c)}};se.Scene.prototype.render=function(a){this.updatePivot(a);this.clearframe(a);this.renderBackground(a);for(var b in this.objs)this.objs[b].render(a)};
se.Scene.prototype.renderBackground=function(a){a.fillStyle=this.backcolor;a.fillRect(this.pivot.position.x,this.pivot.position.y,this.getWidth(),this.getHeight())};se.Scene.prototype.clearframe=function(a){a.clearRect(this.pivot.position.x,this.pivot.position.y,this.getWidth(),this.getHeight())};se.Scene.prototype.updatePivot=function(a){a.setTransform(1,0,0,1,0,0);var b=this.camera.transform.position,c=b.x-this.getWidth()/2,b=b.y-this.getHeight()/2;this.pivot.change(c,b);a.translate(-c,-b)};
...
se.StarEngine.prototype.update=function(){var a=Date.now(),b=(a-this.lastTime)/1E3,c=this.getSceneCurrent();c.update(b);c.render(this.ctx);this.lastTime=a};se.StarEngine.prototype.init=function(){se.windowGameMain=this;animationFrame(se.updateFrame)};
