...
se.Extent.prototype.getIntersection=function(a){var b=se.Extent.createEmpty();if(this.intersects(a))b.min.x=this.min.x>a.min.x?this.min.x:a.min.x,b.min.y=this.min.y>a.min.y?this.min.y:a.min.y,b.max.x=this.max.x<a.max.x?this.max.x:a.max.x,b.max.y=this.max.y<a.max.y?this.max.y:a.max.y;else return null;return b};se.Extent.prototype.getWidth=function(){return this.max.x-this.min.x};se.Extent.prototype.getHeight=function(){return this.max.y-this.min.y};se.factory={};se.factory.rect=function(a){var b=a||{},c=b.name||"",d=b.x||0,e=b.y||0;a=b.w||10;var f=b.h||10,g=b.image_src,h=b.fillColor||"#6B4226",b=b.objoptions||{};b.vertices=[new se.Vector(0,0),new se.Vector(a,0),new se.Vector(a,f),new se.Vector(0,f)];c=new se.GameObject(c,d,e,b);c.setRigidBody(new se.RigidBodyMatter({vertices:b.vertices}));g?c.setRenderer(new se.ImageRenderer(g,a,f)):c.setRenderer(new se.RectRenderer(h,a,f));return c};
se.factory.circle=function(a){var b=a||{},c=b.name||"",d=b.x||0,e=b.y||0,f=a.radius||10,g=b.objoptions||{};g.vertices=se.factory.createCircleVertices(f,a.maxSides||25);a=b.fillColor;var h=b.strokeColor,b=b.lineWidth,c=new se.GameObject(c,d,e,g);c.setRigidBody(new se.RigidBodyMatter(g));c.setRenderer(new se.CircleRenderer(f,a,h,b));return c};
se.factory.createCircleVertices=function(a,b){b=options.maxSides||25;var c=Math.ceil(Math.max(10,Math.min(b,a)));1===c%2&&(c+=1);for(var d=2*Math.PI/c,e="",f=.5*d,g=0;g<c;g+=1)var h=f+g*d,k=Math.sin(h)*a,e=e+("L "+(Math.cos(h)*a).toFixed(3)+" "+k.toFixed(3)+" ");return Matter.Vertices.fromPath(e)};se.GameObject=function(a,b,c,d){this.name=a;this.transform=new se.Transform(this,b,c);this.components=[];this.colliders=[];this.parent=this.renderer=null;this.children=[];d=d||{};this.isStatic=null!=d.isStatic?d.isStatic:!1;this.isSleeping=d.isSleeping||!1;this.canRotate=null!=d.canRotate?d.canRotate:!0;this.mesh=d.mesh||new se.Mesh({vertices:d.vertices});this.rigidbody=d.rigidbody||null;this.angle=0};se.GameObject.prototype.getX=function(){return this.transform.position.x};
se.GameObject.prototype.getY=function(){return this.transform.position.y};se.GameObject.prototype.getWidth=function(){return this.width};se.GameObject.prototype.getHeight=function(){return this.height};se.GameObject.prototype.getColliders=function(){return this.colliders};se.GameObject.prototype.setRenderer=function(a){a.setParent(this);this.renderer=a};
...
this.color;this.strokeStyle&&(a.lineWidth=this.lineWidth,a.strokeStyle=this.strokeStyle);a.fill();a.stroke()};se.RigidBodyMatter=function(a){this.options=a};se.RigidBodyMatter.prototype.createBody=function(){var a=this.parent,b=a.transform.position;options=this.options||{};var c=options.x||b.x,b=options.y||b.y;delete options.x;delete options.y;delete options.width;delete options.height;options.isStatic=a.isStatic;options.canRotate=a.canRotate;a={label:"Rectangle Body",position:{x:c,y:b},vertices:a.mesh.getVertices()};this.body=Matter.Body.create(Matter.Common.extend({},a,options))};
