...
se.Transform.prototype.resolveCollision=function(a,b){var c=this.parent,d=c.parent.getObjs(),e=c.getColliders(),g;for(g in e){var f=e[g],k;for(k in d){var h=d[k];c!=h&&(h=f.getIntersection(h.getColliders()))&&(0!=a&&(this.position.x=0>a?this.position.x+(h.getWidth()+.01):this.position.x-(h.getWidth()+.01)),0!=b&&(this.position.y=0>b?this.position.y+(h.getHeight()+.01):this.position.y-(h.getHeight()+.01)))}}};
se.Transform.prototype.canMove=function(a,b){var c=this.parent,d=c.parent.getObjs(),e=c.getColliders(),g;for(g in e){var f=e[g].clone();f.setParent(c);f.x+=a;f.y+=b;for(var k in d){var h=d[k];if(c!=h&&f.isIntersect(h.getColliders()))return!1}}return!0};se.Transform.prototype.getXY=function(){var a=this.position.x,b=this.position.y,c=this.parent.parent;c instanceof se.GameObject&&(a+=c.transform.position.x,b+=c.transform.position.y);return{x:a,y:b}};
...
se.Scene.prototype._resolveColliders=function(a){for(var b=[],c=0;c<a.length;c++){var d=a[c];if(!d.isStatic&&!d.isSleeping)for(var e=0;e<a.length;e++)if(c!=e){var g=a[e],f=this._checkCollision(d,g);f&&b.push(f)}}for(c in b)if(f=b[c],f.objB.isStatic||g.isSleeping)a=f.objA.rigidbody.velocity,d=f.objA.transform.position,d.y=0<a.y?d.y-(f.intersection.getHeight()+.01):d.y+(f.intersection.getHeight()+.01),a.x=0,a.y=0};
se.Scene.prototype._checkCollision=function(a,b){var c=a.getColliders(),d=b.getColliders(),e;for(e in c){var g=c[e];if(g.isIntersect(d))return new se.Collision(a,b,g.getIntersection(d))}};se.Scene.prototype.render=function(a){this.updatePivot(a);this.clearframe(a);this.renderBackground(a);for(var b in this.objs)this.objs[b].render(a)};se.Scene.prototype.renderBackground=function(a){a.fillStyle=this.backcolor;a.fillRect(this.pivot.position.x,this.pivot.position.y,this.getWidth(),this.getHeight())};
