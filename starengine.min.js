var se={};"undefined"!==typeof window&&(window.requestAnimationFrame=function(){return window.requestAnimationFrame||window.webkitRequestAnimationFrame||window.mozRequestAnimationFrame||window.oRequestAnimationFrame||window.msRequestAnimationFrame||function(a){window.setTimeout(function(){a(new Date.now)},1E3/60)}}(),window.cancelAnimationFrame=window.cancelAnimationFrame||window.mozCancelAnimationFrame);se.RectCollider=function(a,b,c,d){this.x=a;this.y=b;this.width=c;this.height=d};se.RectCollider.createByExtent=function(a){return new se.RectCollider(a.min.x,a.min.y,a.max.x,a.max.y)};se.RectCollider.prototype.setParent=function(a){this.parent=a};se.RectCollider.prototype.getExtent=function(){var a=this.parent.transform.getRealPosition();return new se.Extent(a.x,a.y,a.x+this.width,a.y+this.height)};se.RectCollider.prototype.isIntersect=function(a){var b=this.getExtent();a=a.getExtent();return b.intersects(a)};
se.RectCollider.prototype.getIntersection=function(a){if(a instanceof Array)for(var b in a){var c=a[b];if(this._isIntersect(c))return this._getIntersection(c)}else return this._getIntersection(a);return null};se.RectCollider.prototype._getIntersection=function(a){var b=this.getExtent();a=a.getExtent();return b.getIntersection(a)};se.RectCollider.prototype.clone=function(){return new se.RectCollider(this.x,this.y,this.width,this.height)};se.ComponentScript=function(a){this.update=a};se.ComponentScript.prototype.setParent=function(a){this.parent=a};se.ComponentPlatformPlayerController=function(a,b,c,d){this.joystick=a;this.speed=b};
...
se.ComponentFollowObject.prototype.update=function(a){a.transform.position.x=this.obj_target.transform.position.x;a.transform.position.y=this.obj_target.transform.position.y};se.ComponentFollowObject.prototype.setParent=function(a){this.parent=a};se.Extent=function(a,b,c,d){this.min={x:a,y:b};this.max={x:c,y:d}};se.Extent.createEmpty=function(){return new se.Extent(Infinity,Infinity,-Infinity,-Infinity)};se.Extent.prototype.clone=function(){return new se.Extent(this.min.x,this.min.y,this.max.x,this.max.y)};se.Extent.prototype.move=function(a){this.min.x=a.x;this.min.y=a.y;this.max.x+=a.x;this.max.y+=a.y;return this};
se.Extent.prototype.extend=function(a){a.min.x<this.min.x&&(this.min.x=a.min.x);a.max.x>this.max.x&&(this.max.x=a.max.x);a.min.y<this.min.y&&(this.min.y=a.min.y);a.max.y>this.max.y&&(this.max.y=a.max.y)};se.Extent.prototype.extendVector=function(a){a.x<this.min.x&&(this.min.x=a.x);a.x>this.max.x&&(this.max.x=a.x);a.y<this.min.y&&(this.min.y=a.y);a.y>this.max.y&&(this.max.y=a.y)};se.Extent.prototype.extendVectors=function(a){for(var b in a)this.extendVector(a[b])};
...
se.Extent.prototype.getWidth=function(){return this.max.x-this.min.x};se.Extent.prototype.getHeight=function(){return this.max.y-this.min.y};se.factory={};
...
se.factory.createCircleVertices=function(a,b){var c=Math.ceil(Math.max(10,Math.min(b||25,a)));1===c%2&&(c+=1);for(var d=2*Math.PI/c,e="",f=.5*d,g=0;g<c;g+=1)var k=f+g*d,h=Math.sin(k)*a,e=e+("L "+(Math.cos(k)*a).toFixed(3)+" "+h.toFixed(3)+" ");return Matter.Vertices.fromPath(e)};se.GameObject=function(a,b,c,d){this.name=a;this.transform=new se.Transform(this,b,c);this.components=[];this.colliders=[];this.parent=this.renderer=null;this.children=[];d=d||{};this.isStatic=null!=d.isStatic?d.isStatic:!1;this.isSleeping=d.isSleeping||!1;this.canRotate=null!=d.canRotate?d.canRotate:!0;this.mesh=d.mesh||new se.Mesh(this,{vertices:d.vertices});this.rigidbody=d.rigidbody||null;this.angle=0};se.GameObject.prototype.getX=function(){return this.transform.position.x};
...
se.KeyboardHandler.prototype.keyup=function(a){"65"==a||"68"==a||"37"==a||"39"==a?this.joy.setAxis("horizontal",0):"87"==a||"83"==a?this.joy.setAxis("vertical",0):"32"==a&&this.joy.setAxis("jump",!1)};se.Mesh=function(a,b){this.parent=a;b=b||{};this.vertices=b.vertices||[]};se.Mesh.prototype.setVertices=function(a){this.vertices=a};se.Mesh.prototype.getVertices=function(){return this.vertices};se.Mesh.prototype.getExtent=function(){var a=se.Extent.createEmpty();a.extendVectors(this.vertices);return a};se.RectRenderer=function(a,b,c){this.color=a;this.width=b;this.height=c};se.RectRenderer.prototype.render=function(a){a.fillStyle=this.color;a.fillRect(-this.width/2,-this.height/2,this.width,this.height)};se.RectRenderer.prototype.setParent=function(a){this.parent=a};se.CircleRenderer=function(a,b,c,d){this.radius=a;this.fillStyle=b;this.strokeStyle=c;this.lineWidth=d||1};
