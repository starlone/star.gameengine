...
se.BoxCollider.prototype.clone=function(){return new se.BoxCollider(this.x,this.y,this.width,this.height)};se.Extent=function(a,b,c,d){this.min={x:a,y:b};this.max={x:c,y:d}};se.Extent.createEmpty=function(){return new se.Extent(Infinity,Infinity,-Infinity,-Infinity)};se.Extent.prototype.isEmpty=function(a){return this.max.x<this.min.x||this.max.y<this.min.y};se.Extent.prototype.intersects=function(a){return this.min.x<=a.max.x&&this.max.x>=a.min.x&&this.min.y<=a.max.y&&this.max.y>=a.min.y};
se.Extent.prototype.getIntersection=function(a){var b=se.Extent.createEmpty();if(this.intersects(a))b.min.x=this.min.x>a.min.x?this.min.x:a.min.x,b.min.y=this.min.y>a.min.y?this.min.y:a.min.y,b.max.x=this.max.x<a.max.x?this.max.x:a.max.x,b.max.y=this.max.y<a.max.y?this.max.y:a.max.y;else return null;return b};se.Extent.prototype.getWidth=function(){return this.max.x-this.min.x};se.Extent.prototype.getHeight=function(){return this.max.y-this.min.y};se.Scene=function(a){this.camera=new se.GameObject("MainCamera",0,0,0,0);this.camera.setParent(this);this.pivot=new se.Transform(this,0,0);this.objs=[this.camera];this.backcolor=a||"rgba(135, 206, 250,1)"};se.Scene.prototype.getWidth=function(){return this.parent.getWidth()};se.Scene.prototype.getHeight=function(){return this.parent.getHeight()};se.Scene.prototype.getCamera=function(){return this.camera};se.Scene.prototype.getObjs=function(){return this.objs};
se.Scene.prototype.setParent=function(a){this.parent=a};se.Scene.prototype.add=function(a){this.objs.push(a);a.setParent(this)};se.Scene.prototype.update=function(a){for(var b in this.objs)this.objs[b].update(a)};se.Scene.prototype.render=function(a){this.updatePivot(a);this.clearframe(a);this.renderBackground(a);for(var b in this.objs)this.objs[b].render(a)};
